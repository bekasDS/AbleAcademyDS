...
def draw_predict_scatter(models_predictions:DataFrame, df_metrics:DataFrame,
    ...
    # название колонок
    colname_list = models_predictions.columns.to_list()

    num_cell = ceil((len(colname_list)-1)**0.5)         # количество ячеек сетки , например "2x2"

    plt.tight_layout(pad=0, w_pad=0.5, h_pad=4)
...
def combination_polinomial(df_train:DataFrame, df_test:DataFrame,
    ...
    # валидация
    df_multiple_polunomial_linear = multiple_polynomial_linear_validation(model=multiple_polunomial_linear_regressor,
                                                                                                        data=df_test, sc_train_y=sc_train_y)
    # метрики
...
def combination_forest(df_train:DataFrame, df_test:DataFrame, sc_train_y:StandardScaler,
    ...
    # валидация
    df_forest = random_forest_validation(model=forest_regressor, data=df_test, sc_train_y=sc_train_y)
    # метрики
...
def combination_decision_tree(df_train:DataFrame, df_test:DataFrame, sc_train_y:StandardScaler,
    ...
    """
    # регрессор
    decision_tree_regressor = train_decision_tree_regressor(data=df_train, parameters=parameters,
                                                                                     degree=degree, random_state=random_state)
    # валидация
    df_decision_tree = random_forest_validation(model=decision_tree_regressor, data=df_test, sc_train_y=sc_train_y)
    # метрики
...
def combination_svm(df_train:DataFrame, df_test:DataFrame, sc_train_y:StandardScaler,
    ...
    # валидация
    df_svm = svm_validation(regressor=svm_regressor, data=df_test, sc_train_y=sc_train_y)
    # метрики
...
def combination_gnb(df_train:DataFrame, df_test:DataFrame, parameters:list, sc_train_y:StandardScaler) -> list:
    ...
    """
    # регрессор
    gnb_regressor = train_gnb_regressor(df_data=df_train, parameters=parameters, sc_train_y=sc_train_y)
    # валидация (функция одинаковая для линейной регрессии)
    df_gnb = linear_validation(regressors=gnb_regressor, data=df_test, sc_train_y=sc_train_y,
                               important=parameters, isSimple=False, isScale=False)
    # метрики
...
def combination_iso(df_train:DataFrame, df_test:DataFrame, parameters:list, sc_train_y:StandardScaler) -> list:
    ...
    # валидация
    df_iso = iso_validation(regressors=iso_regressor, data=df_test, important=parameters, sc_train_y=sc_train_y)
